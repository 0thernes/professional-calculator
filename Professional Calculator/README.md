...
[![tests](https://img.shields.io/badge/tests-1075%20passing-brightgreen)](#testing)
...
- **Every algorithm is anchored to a closed-form check in the test suite.** Not "looks plausible" — `det(AB) = det(A)·det(B)`, eigenvalues of a rotation matrix come back `±i`, `∫₀^π sin x dx = 2`, `Φ(1.96) = 0.975`, Black–Scholes obeys put–call parity. 1075 tests, all green.
...
| **Units** | 7-dimension SI analysis, conversion, affine temperature scales, **unit-aware expressions** (`3 kg · 9.8 m/s²`) | [`units.js`](math/units.js) · [`unitexpr.js`](math/unitexpr.js) |
...
| **STEM Lab** | paged live-SVG visual panel (11 cyclable scientific visualizations) | [`stem.js`](stem.js) |
...
existing `nCr`/`nPr`, `isprime`, `totient`, `fib`, plus bit ops `popcount`/`bitlength`/`gray`/`igray`/`hamming`, …).
...
```
36 test suites · 1075 tests · 100% pass
